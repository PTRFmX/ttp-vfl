...
LinearRegression::LinearRegression(bool is_player, RowMatrixXi64 X, ColVectorXi64 Y, 
...
void LinearRegression::train() {
    ...
    if (train_DA) {
        if (is_player) {
            player->train_linear_DA_simulate(learning_rate);
        } else {
            // coord->train_linear();
        }
    }
        ...
        if (is_player) {
            player->train_linear(learning_rate);
            // player->reshare();
        } else {
            coord->train_linear();
        }
